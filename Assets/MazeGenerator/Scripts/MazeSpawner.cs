...
//NEW CODE VDMS
                ...
                {
                    AutomaticMovement.MazeMatrix[(row * 2) + 1][(column * 2) + 1 + 1] = 1;
                    AutomaticMovement.MazeMatrix3DPoints[(row * 2) + 1][(column * 2) + 1 + 1] = new Vector3(x + CellWidth / 2, 0, z) + Wall.transform.position;
//NEW CODE VDMS
                }
...
/*NEW CODE VDMS*/
                ...
                {
                    AutomaticMovement.MazeMatrix[(row * 2) + 1 + 1][(column * 2) + 1] = 1;
                    AutomaticMovement.MazeMatrix3DPoints[(row * 2) + 1 + 1][(column * 2) + 1] = new Vector3(x, 0, z + CellHeight / 2) + Wall.transform.position;
/*NEW CODE VDMS*/
                }
...
/*NEW CODE VDMS*/
                ...
                {
                    AutomaticMovement.MazeMatrix[(row * 2) + 1][(column * 2)] = 1;
                    AutomaticMovement.MazeMatrix3DPoints[(row * 2) + 1][(column * 2)] = new Vector3(x - CellWidth / 2, 0, z) + Wall.transform.position;
/*NEW CODE VDMS*/
                }
                ...
                {
                    AutomaticMovement.MazeMatrix[(row * 2)][(column * 2) + 1] = 1;
                    AutomaticMovement.MazeMatrix3DPoints[(row * 2)][(column * 2) + 1] = new Vector3(x, 0, z - CellHeight / 2) + Wall.transform.position;
/*NEW CODE VDMS*/
                }
            }
        }
        ...
        if (Pillar != null)
        {
            for (int row = 0; row < Rows + 1; row++)
            {
                for (int column = 0; column < Columns + 1; column++)
                {
                    float x = column * (CellWidth + (AddGaps ? .2f : 0));
                    float z = row * (CellHeight + (AddGaps ? .2f : 0));
                    GameObject tmp = Instantiate(Pillar, new Vector3(x - CellWidth / 2, 0, z - CellHeight / 2) + Pillar.transform.position, Pillar.transform.localRotation) as GameObject;
                    tmp.transform.parent = transform;
                }
            }
        }
        ...
        if (GoalPrefab != null){
            ...
            for(uint i = 0; i < NUMOFGOALS; ++i){
                GameObject tmp;
                int RandomX = Random.Range(0, Columns), RandomY = Random.Range(0,Rows);
                while ((RandomX == 0) && (RandomY == 0)){
                    RandomX = Random.Range(0, Columns);
                    RandomY = Random.Range(0, Rows);
                }
                tmp = Instantiate(GoalPrefab, new Vector3(Random.Range(0, Columns) * (CellWidth + (AddGaps ? .2f : 0)), 1, Random.Range(0, Rows) * (CellHeight + (AddGaps ? .2f : 0))), Quaternion.Euler(0, 0, 0)) as GameObject;
                tmp.transform.parent = transform;
            }
        }
    }
